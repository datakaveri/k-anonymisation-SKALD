...
def main_process(config):
    ...
    if "chunking" in operations:
        try:
            ...
            # 💾 Save a copy of the generated YAML for debugging
            ...
            # Read anonymized output of first chunk
            output_file = "generalized_chunk1.csv"
            ...
            # Find all chunk files in numeric order
            fileList = sorted(glob.glob(os.path.join(chunk_directory, "*.csv")), key=extract_chunk_number)
            if not fileList:
                raise ValueError(f"No chunk files found in directory: {chunk_directory}")

            # First chunk will be the numerically smallest
            preview_chunk_path = fileList[0]
            first_chunk_df = pd.read_csv(preview_chunk_path)
            ...
            for chunk_path in fileList[:number_of_chunks]:
                ...
                if response.status_code == 200:
                    ...
                    if not saved_first_chunk:
                        # Save ARX output JSON for the first chunk only
                        arx_preview = result
            ...
            base_chunk_path = preview_chunk_path.replace("chunk1.csv", "chunk{i}.csv")
            k_anon_params.pop("dataset_name", None)
            best_node = average_best_node(
                arx_ready_config=arx_ready_config,
                num_chunks=number_of_chunks,
                url=url,
                headers=headers,
                base_chunk_path=base_chunk_path,
                params_template=k_anon_params
            )

            print(f"[ARX] Final averaged node across chunks: {best_node}")

            dm_star_value, arx_eq_classes = calculate_DM(
                config,
                arx_ready_config,
                number_of_chunks,
                preview_chunk_path,
            ...
            concat_output = {
                ...
                "arx_output_json": arx_preview,
                "first_chunk_data": first_chunk_df.to_dict(orient="records"),
                "status": "success",
                "status_code": "0000"
            }
